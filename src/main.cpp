...
Main::Main() {
  hookCChatBubble__Draw.set_cb(
      [this](const decltype(hookCChatBubble__Draw)& hook,
             samp::CChatBubble* p_this) -> void {
        return this->CChatBubble__Draw(hook, p_this);
      });
  hookCChatBubble__Draw.install();

  hookCChatBubble__Add.set_cb(
      [this](const auto& hook, samp::CChatBubble* p_this, uint16_t n_Player,
             const char* szText, uint32_t color, float fDrawDistance,
             int lifeSpan) -> int16_t {
        return this->CChatBubble__Add(hook, p_this, n_Player, szText, color,
                                      fDrawDistance, lifeSpan);
      });
...
int16_t Main::CChatBubble__Add(const decltype(hookCChatBubble__Add)& hook,
                               samp::CChatBubble* p_this, uint16_t n_Player,
                               const char* szText, uint32_t color,
                               float fDrawDistance, int lifeSpan) {
  ...
  if (!strcmp(copyText, bubbles[0].m_szText)) {
    ...
    bubble = &*std::min_element(
        bubbles.begin(), bubbles.end(),
        [lastTick](const auto& a, const auto& b) {
  ...
  auto orig = hook.get_trampoline()(p_this, n_Player, szText, color,
                                    fDrawDistance, lifeSpan);
...
void Main::TryInitImgui() {
  bool isSAMPInitialized =
      *reinterpret_cast<void**>(sampapi::GetBase() + 0x21A0F8) != nullptr;
  if (isImguiInitialized || !isSAMPInitialized) return;

  HWND* hwnd = *reinterpret_cast<HWND**>(0xC17054);
  IDirect3DDevice9* pDevice = *reinterpret_cast<IDirect3DDevice9**>(0xC97C28);

  if (!hwnd || !pDevice) return;
...
void Main::CChatBubble__Draw(const decltype(hookCChatBubble__Draw)& hook,
  ...
  if (samp::RefGame() && samp::RefGame()->GetPlayerPed() &&
      samp::RefNetGame()) {
    if (samp::RefLabel()) samp::RefLabel()->Begin();
    ...
    for (int i = 0; i < largestId; i++) {
      ...
      for (auto& bubble : g_Bubbles[i]) {
        if (!bubble.m_bExists) continue;

        if (distanceToPlayer > bubble.m_fDrawDistance) continue;
        ...
        calcTotalHeight(utf8_ChatBubbleText, settings.getMaxWidth(),
                        totalHeight, lines);
        ...
        if (vecScreen.z >= 0) {
          for (auto& line : lines) {
            ...
            uint32_t color =
                fadeInOut(lastTick, bubble.m_creationTick, bubble.m_lifeSpan,
                          settings.getDuration(), bubble.m_color);
    ...
    if (samp::RefLabel()) samp::RefLabel()->End();
  }
